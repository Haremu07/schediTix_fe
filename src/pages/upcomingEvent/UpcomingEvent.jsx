    ...
    const handleAllEvent = async () => {
      try {
        const response = await axios.get(
          `${BASEURL}/api/v1/user/events/upcoming`,
          { headers }
        );
  ...
  return (
    <div className="tickets-container">
      ...
      <div className="tickets-list">
        {allEvent?.filter((e) => e.status === "upcoming").length === 0
          ? "No upcoming"
          : upcomingEvents?.map((ticket, index) => (
              <div
                ...
                key={index}
                onClick={() =>
                  navigate(`/dashboard/upcoming-event-details/${ticket._id}`)
                }
              >
                <div className="ticket-image">
                  <img
                    src={ticket?.image.imageUrl || image1}
